...
This model [originates](/model/about-model) from the time when I founded and operated Praqma. In this post, I'll describe how I conducted this Continuous Delivery Metric Assessment at more then 25 companies
{: .kicker}
...
>_Imagine that the principle described on the card was fully implemented in your process. How much would it affect [throughput, feedback and payback](/model/gauges/) — and how complex would the implementation be_
...
The findings are then recorded and presented as a detailed and customized note on each card - see card ["Automated Builds"](/model/automated-builds/) as an example (these are real finding from some of the assessments, but anonymized and used here to serve as examples). Also notice on the ["Automated Builds"](/model/automated-builds/) card, that the scoring from the workshop is captured. The score is 7 and [complexity is categorized](/model/gauges/) as _a group effort_.
